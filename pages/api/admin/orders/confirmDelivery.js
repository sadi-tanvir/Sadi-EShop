...
const handler = async (req, res) => {
    try {
        ...
        const order = await Order.findOneAndUpdate({ _id: req.body._id }, {$set:{shipping: true}}, { new: true });
        if (!order) return res.status(400).json({ message: 'Failed to update Delivery status' });
