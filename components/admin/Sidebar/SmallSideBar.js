...
const SmallSideBar = ({ sidebarHandler, sideBar, menuItemText }) => {
    ...
    return (
        <>
            <div className={`${sideBar ? " translate-x-0" : " -translate-x-64"} min-h-screen w-64 z-40 absolute bg-gray-100 shadow-2xl md:h-full flex-col justify-between sm:hidden transition duration-150 ease-in-out`} id="mobile-nav">
                ...
                <div className="px-8">
                    <div className="h-16 w-full flex items-center mt-2">
                                        ...
                                        <Link href={elem.pathname}>
                                            <li className={`${router.pathname == elem.pathname ? "bg-primary shadow-md" : ""} py-2 px-2 rounded-md flex w-full justify-between text-gray-300 hover:text-gray-500 cursor-pointer items-center mb-6`}>
                                                <a href="javascript:void(0)" className="flex items-center focus:outline-none focus:ring-2 focus:ring-white">
                                                    {elem.icon}
                                                    <span className={`${router.pathname == elem.pathname ? 'text-white' : 'text-secondary'} text-sm ml-2`}>
                                                        {elem.pageName}
                                                    </span>
                                                </a>
                ...
                <div className="absolute bottom-0 w-full px-8 border-t border-gray-300">
